...
Route::('/logout', [LoginController::class, 'destroy'])->name('logout')->middleware('auth');
...
Route::middleware('auth:web')->group(function () {
    ...
    // 勤怠登録機能（出勤）
    Route::post('/record-attendance', [AttendanceController::class, 'recordAttendance'])->name('record.attendance'); // name追加
...
Route::middleware(['auth:admin', 'can:access-admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    // スタッフ別勤怠一覧(管理)
    // {id} で動的な値を受け取れるように修正
    Route::get('/attendances/staff/{id}',function () {
        return view('admin.staff_attendance_list');
    })->name('attendances.staff.id'); // name() には admin.attendances.staff.id が自動で付与される
...
// --- 両方のユーザー（web または admin）がアクセスできるルート ---
// ログインしているのが web ガードのユーザーか admin ガードのユーザーかを判別し、
// それぞれに応じた処理を行う必要があります。
