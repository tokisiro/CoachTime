...
Route::middleware(['auth'])->group(function () {
    Route::get('/dashboard', function () {
        return view('normal.attendance_register'); // または 'dashboard' など、適切なビュー名を指定
...
Route::middleware(['auth:admin', 'can:access-admin'])->prefix('admin')->name('admin.')->group(function () {
    Route::get('/dashboard', function () {
        return redirect()->route('admin.attendances.list'); // 管理者ダッシュボードのビュー
    })->name('dashboard');
});
...
//勤怠登録画面
    //出勤登録画面(一般)
    Route::get('/attendance', [AttendanceController::class, 'showAttendanceRegister'])->name('attendance');
