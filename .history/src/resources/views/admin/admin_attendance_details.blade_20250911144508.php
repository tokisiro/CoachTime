...
<div class="attendance-details">
    ...
    @if($breakErrors->isNotEmpty())
        <tr class="attendance-details__error-row"> {{-- エラー行用のクラスを追加 --}}
            <th>ERROR (休憩{{ $index + 1 }})</th> {{-- どの休憩のエラーか分かりやすく --}}
            <td colspan="3"> {{-- 複数のセルを結合してメッセージを表示 --}}
                @foreach ($breakErrors as $message)
                    <p class="error-message">{{ $message }}</p> {{-- 各メッセージをパラグラフで囲む --}}
                @endforeach
            </td>
        </tr>
    @endif
