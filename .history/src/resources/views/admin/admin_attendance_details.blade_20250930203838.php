    ...
    <form class="attendance-details__parts" action="{{ route('admin.approve', ['id' => $attendance->id]) }}" method="POST" id="attendance-form">
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr>
                ...
                value="{{ $checkInTimeValue }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input" type="text" name="closing_time"
                value="{{ $closingTimeValue }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}></td>
            </tr>
            @error('check_in_time')
            <tr>
                <th>ERROR</th>
            ...
            <tr class="attendance-details__parts-table--existing">
                <th>休憩{{ $index + 1 }}</th>
                <td>
                    <input type="hidden" name="breaks[{{ $index }}][id]" value="{{ 'breaks.' . $index . '.start_time',$break->id }}">
                    <input class="attendance-details__parts-table--input break-start-time" type="text" name="breaks[{{ $index }}][start_time]"
                    value="{{ $break->start_time_value }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}>
                </td>
                    <td class="attendance-details__parts-table--td">〜</td>
                <td>
                    <input class="attendance-details__parts-table--input break-end-time" type="text" name="breaks[{{ $index }}][end_time]"
                    value="{{'breaks.' . $index . '.end_time', $break->end_time_value }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}>
                </td>
            </tr>
            @php
            ...
            <tr class="attendance-details__parts-table--new" style="{{ ($isPendingApproval || $isAttendanceFinalized) ? 'display: none;' : '' }}">
                ...
                value="{{ $newBreakStartTimeValue }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input new-break-end-time" type="text" name="new_breaks[0][end_time]"
                value="{{ $newBreakEndTimeValue }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}></td>
            </tr>
            ...
            <tr>
                <th>備考</th>
                <td colspan="2"><textarea class="attendance-details__parts-table--remarks" name="proposed_remarks"
                {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}>{{ $remarksValue }}</textarea>
                </td>
            </tr>
