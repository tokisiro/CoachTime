...
<div class="attendance-list">
    <h2 class="attendance-list__title">
        {{ \Carbon\Carbon::parse($selectedMonth)->format('Y年m月') }}の勤怠
    </h2>
    <div>
        {{-- 日付選択機能を追加する場合 --}}
        <form action="{{ route('admin.attendances.list') }}" method="GET" class="attendance-list__date">
            <p>
                <button type="submit" name="month" class= "attendance-list__date-previous" value="{{ \Carbon\Carbon::parse($selectedMonth)->subMonth()->format('Y-m-d') }}">
                    <-前日
                </button>
            </p>
            <div class="attendance-list__date-container">
                ...
                <div>
                    {{ \Carbon\Carbon::parse($selectedMonth)->format('Y/m/d') }}
                </div>
            </div>
            <p>
                <button type="submit" name="month" value="{{ \Carbon\Carbon::parse($selectedMonth)->addMonth()->format('Y-m-d') }}"
    ...
    @forelse ($attendancesByDate as $date => $attendances)
    <h3 class="attendance-list__date-heading">
