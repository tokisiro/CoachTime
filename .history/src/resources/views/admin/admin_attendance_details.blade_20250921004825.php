...
<div class="attendance-details">
    <form class="attendance-details__parts" action="{{ route('admin.approve', ['id' => $attendance->id]) }}" method="POST" id="attendance-form">
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr>
                ...
                value="{{ old('check_in_time', $isPendingApproval
                   ...
                   // 承認待ちの申請がない場合
                    ...
                    $latestApprovedApplication && $latestApprovedApplication->proposed_check_in_time
    ...
    $start_time_value = '';
    if ($break->start_time) {
        $start_time_value = \Carbon\Carbon::parse($break->start_time)->format('H:i');
    } elseif ($break->proposed_start_time) {
        $start_time_value = \Carbon\Carbon::parse($break->proposed_start_time)->format('H:i');
    }
?>
                    <input class="attendance-details__parts-table--input break-start-time" type="text" name="breaks[{{ $index }}][start_time]" value="{{ old('breaks[{{ $index }}][start_time]',$break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : ($break->proposed_start_time ? \Carbon\Carbon::parse($break->proposed_start_time)->format('H:i') : '' ))}}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}>
                </td>
                    <td class="attendance-details__parts-table--td">〜</td>
                <td>
                    <input class="attendance-details__parts-table--input break-end-time" type="text" name="breaks[{{ $index }}][end_time]" value="{{ old('breaks[{{ $index }}][end_time]',$break->end_time ? \Carbon\Carbon::parse($break->end_time)->format('H:i') : ($break->proposed_end_time ? \Carbon\Carbon::parse($break->proposed_end_time)->format('H:i') : '') }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}>
                </td>
            </tr>
            @php
            ...
            <tr class="attendance-details__parts-table--new" style="{{ ($isPendingApproval || $isAttendanceFinalized) ? 'display: none;' : '' }}">
                ...
                value="{{old('new_breaks[0][start_time]', $isPendingApproval && !empty($pendingApplication->new_proposed_breaks[0])
                ...
                value="{{old('new_breaks[0][end_time]',  $isPendingApproval && !empty($pendingApplication->new_proposed_breaks[0])
            ...
            <tr>
                <th>備考</th>
                <td colspan="2"><input class="attendance-details__parts-table--remarks" type="textarea" name="remarks"
                value="{{ {{old(remarks)}}$isPendingApproval
