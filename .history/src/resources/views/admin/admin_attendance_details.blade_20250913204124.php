        ...
        <table class="attendance-details__parts-table">
            ...
            <tr class="attendance-details__parts-table--existing">
                ...
                <td>
                    ...
                    <input class="attendance-details__parts-table--input break-start-time" type="text" name="breaks[{{ $index }}][start_time]" value="{{ $break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : '' }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'readonly' : '' }}>
                </td>
                    <td class="attendance-details__parts-table--td">〜</td>
                <td>
            ...
            <tr class="attendance-details__parts-table--new" style="{{ ($isPendingApproval || $isAttendanceFinalized) ? 'display: none;' : '' }}">
                ...
                value="{{$isPendingApproval && !empty($pendingApplication->new_proposed_breaks[0])
                        ...
                        ? (\Carbon\Carbon::parse($latestApprovedApplication->new_proposed_breaks[0]['end_time'])->format('H:i') ?? '')
                           : '' // どちらでもない場合は空
            )}}" {{ $isPendingApproval || $isAttendanceFinalized ? '' : '' }}></td>
            </tr>
