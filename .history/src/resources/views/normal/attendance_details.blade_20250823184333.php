    ...
    <form class="attendance-details__parts" action="{{ route('attendance.apply_edit', ['id' => $attendance->id]) }}" method="POST">
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr>
                ...
                <td><input class="attendance-details__parts-table--input" type="text" name="breaks[{{ $index }}][start_time]" value="{{ $break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : '' }}"></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input" type="text" name="breaks[{{ $index }}][end_time]" value="{{ $break->end_time ? \Carbon\Carbon::parse($break->end_time)->format('H:i') : '' }}"></td>
            </tr>
            @endforeach
            <tr>
                <th>休憩</th>
                <td><input class="attendance-details__parts-table--input" type="text" name="breaks[{{ $attendance->breaks->count() }}][proposed_start_time]" value=""></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input" type="text" name="breaks[{{ $attendance->breaks->count() }}][proposed_end_time]" value=""></td>
            </tr>
            <tr>
                <th>備考</th>
                <td colspan="2"><input class="attendance-details__parts-table--remarks" type="textarea" name="remarks" value="{{ $attendance->remarks ?? '' }}"></td>
            </tr>
        </table>
