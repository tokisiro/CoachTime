...
<div class="attendance-details">
    <form class="attendance-details__parts" action="{{route('applications', ['id' => $attendance->id]) }}" method="POST" id="attendance-form">
        ...
        <table class="attendance-details__parts-table">
            ...
            @if($break->proposed_start_time || $break->proposed_end_time)
        <tr class="attendance-details__parts-table--proposed">
            <th>提案値</th> {{-- 提案値であることを示すラベル --}}
            <td>
                {{-- 提案値の入力欄は、基本的に編集不可（disabled）にするのが一般的 --}}
                <input class="attendance-details__parts-table--input proposed-break-start-time" type="text"
                value="{{ $break->proposed_start_time ? \Carbon\Carbon::parse($break->proposed_start_time)->format('H:i') : '' }}"
                disabled> {{-- 提案値は表示のみで編集させない --}}
            </td>
            <td class="attendance-details__parts-table--td">〜</td>
            <td>
                <input class="attendance-details__parts-table--input proposed-break-end-time" type="text"
                value="{{ $break->proposed_end_time ? \Carbon\Carbon::parse($break->proposed_end_time)->format('H:i') : '' }}"
                disabled> {{-- 提案値は表示のみで編集させない --}}
            </td>
        </tr>
    @endif
