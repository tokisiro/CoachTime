...
<div class="attendance-register">
    <div class="attendance-register__situation">
        ...
        <div class="attendance-register__situation-button">
            ...
            @endif
            @if ($currentStatus === 'before_work')
                <form action="{{ route('record.attendance') }}" method="POST">
                    @csrf
                    <button class="attendance-register__situation-button--start"  id="startWorkBtn" type="submit">
                        出勤
                    </button>
                </form>
            @endif
            @if ($currentStatus === 'working' && !$hasActiveBreak)
                <form action="{{ route('record.closing_time') }}" method="POST">
                    @csrf
                    <button class="attendance-register__situation-button--end" id="leaveBtn" type="submit">
                        退勤
                    </button>
                </form>
            @endif
