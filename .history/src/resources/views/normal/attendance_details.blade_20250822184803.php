...
<div class="attendance-details">
    <form class="attendance-details__parts" action="{{ route('attendance.apply_edit', ['id' => $attendance->id]) }}" method="POST">
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr>
                <th>休憩{{ $index + 1 }}</th>
                <td><input class="attendance-details__parts-table--input" type="text" name="breaks[{{ $index }}][start_time]" value="{{ $attendance->breaks->get(0) && $attendance->breaks->get(0)->start_time ? \Carbon\Carbon::parse($attendance->breaks->get(0)->start_time)->format('H:i') : '' }}"></td>
                <td class="attendance-details__parts-table--td">〜</td>
