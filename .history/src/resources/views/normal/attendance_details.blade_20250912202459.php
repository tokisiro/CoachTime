    ...
    <form class="attendance-details__parts" action="{{route('applications', ['id' => $attendance->id]) }}" method="POST" id="attendance-form">
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr class="attendance-details__parts-table--existing">
                ...
                value="{{ $break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : '' }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'disabled' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input break-end-time" type="text" name="breaks[{{ $index }}][end_time]"
                value="{{ $break->end_time ? \Carbon\Carbon::parse($break->end_time)->format('H:i') : '' }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'disabled' : '' }}></td>
            </tr>
            @endforeach
            ...
            <tr class="attendance-details__parts-table--new" style="{{ $isPendingApproval ? 'display: none;' : '' }}">
                <th>休憩</th>
                <td><input class="attendance-details__parts-table--input new-break-start-time" type="text" name="new_breaks[0][start_time]" value="{{$isPendingApproval
                        ? (collect($pendingApplication->decoded_proposed_breaks)->firstWhere('id', $break->id)['start_time'] ?? '')
                        : ($latestApprovedApplication
                        ? (collect($latestApprovedApplication->decoded_proposed_breaks)->firstWhere('id', $break->id)['start_time'] ?? '')
                        : ($break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : '')
                        )" {{ $isPendingApproval || $isAttendanceFinalized ? 'disabled' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input new-break-end-time" type="text" name="new_breaks[0][end_time]" value="$isPendingApproval
                        ? (collect($pendingApplication->decoded_proposed_breaks)->firstWhere('id', $break->id)['end_time'] ?? '')
                        : ($latestApprovedApplication
                        ? (collect($latestApprovedApplication->decoded_proposed_breaks)->firstWhere('id', $break->id)['end_time'] ?? '')
