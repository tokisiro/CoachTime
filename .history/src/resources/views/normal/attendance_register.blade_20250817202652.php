...
<div class="attendance-register">
    <div class="attendance-register__situation">
        ...
        <div class="attendance-register__situation-button">
            ...
            <button class="attendance-register__situation-button--break" id="breakBackBtn">休憩戻</button>messageDiv
        </div>
    </div>
</div>
@endsection
...
leaveBtn.addEventListener('click', () => {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            // messageDiv.textContent = 'お疲れ様でした'; // messageDivが定義されていないためコメントアウト
            // ボタンを非表示にするか、またはリセット
            leaveBtn.style.display = 'none';
            breakInBtn.style.display = 'none';
            breakBackBtn.style.display = 'none';
            alert(data.message); // 成功メッセージをユーザーに表示
        } else {
