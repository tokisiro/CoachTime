...
<div class="attendance-details">
    <form class="attendance-details__parts" action="{{ route('attendance.apply_edit', ['id' => $attendance->id]) }}" method="POST" id="attendance-form">
        @csrf
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr class="attendance-details__parts-table--existing">
                ...
                <input class="attendance-details__parts-table--input break-start-time" type="text" name="breaks[{{ $index }}][start_time]" value="{{ $break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : '' }}" {{ $isPendingApproval ? 'disabled' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input break-end-time" type="text" name="breaks[{{ $index }}][end_time]" value="{{ $break->end_time ? \Carbon\Carbon::parse($break->end_time)->format('H:i') : '' }}" {{ $isPendingApproval ? 'disabled' : '' }}></td>
            </tr>
            @endforeach
            <tr class="attendance-details__parts-table--new" style="{{ $isPendingApproval ? 'display: none;' : '' }}">
                <th>休憩</th>
                <td><input class="attendance-details__parts-table--input new-break-start-time" type="text" name="new_breaks[0][start_time]" value="" {{ $isPendingApproval ? 'disabled' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
                <td><input class="attendance-details__parts-table--input new-break-end-time" type="text" name="new_breaks[0][end_time]" value="" {{ $isPendingApproval ? 'disabled' : '' }}></td>
            </tr>
            <tr>
                <th>備考</th>
...
document.addEventListener('DOMContentLoaded', function () {
        ...
        // isPendingApproval が true の場合は JavaScript での制御は不要だが、
        // 念のため disabled 属性をチェックする
        const isFormDisabled = this.querySelector('input[disabled]');
        if (isFormDisabled) {
            // フォーム自体が disabled の場合は、何もしない
            return;
        }

        form.addEventListener('submit', function (event) {
            // 新規追加の休憩行をチェック
            const newBreakRows = document.querySelectorAll('.attendance-details__parts-table--new');
            ...
            // 既存の休憩行をチェック
            const existingBreakRows = document.querySelectorAll('.attendance-details__parts-table--existing');
