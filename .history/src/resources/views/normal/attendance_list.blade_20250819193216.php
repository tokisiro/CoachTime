...
<div class="attendance-list">
    ...
    <div class="attendance-list__date">
        ...
        <p>
            {{ \Carbon\Carbon::parse(request('month', now()))->format('Y年m月') }}
        </p>
        <form action="{{ route('attendances.list') }}" method="GET">
            <p>
    ...
    <table class="attendance-list__table">
        ...
        @foreach ($attendances as $attendance)
            @if ($currentDate != \Carbon\Carbon::parse($attendance->date)->format('Y年m月d日'))
                    @php
                        $currentDate = \Carbon\Carbon::parse($attendance->date)->format('Y年m月d日');
                    @endphp
                    <tr class="attendance-list__table-date-header">
                        <td colspan="7">{{ $currentDate }}</td>
                    </tr>
                @endif
        <tr>
