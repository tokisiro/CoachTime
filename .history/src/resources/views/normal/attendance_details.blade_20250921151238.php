    ...
    <form class="attendance-details__parts" action="{{route('applications', ['id' => $attendance->id]) }}" method="POST" id="attendance-form">
        ...
        <table class="attendance-details__parts-table">
            ...
            <tr class="attendance-details__parts-table--existing">
                ...
                value="{{{{ old('breaks.' . $index . '.start_time', $break->start_time ? \Carbon\Carbon::parse($break->start_time)->format('H:i') : ($break->proposed_start_time ? \Carbon\Carbon::parse($break->proposed_start_time)->format('H:i') : '')) }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'disabled' : '' }}" {{ $isPendingApproval || $isAttendanceFinalized ? 'disabled' : '' }}></td>
                <td class="attendance-details__parts-table--td">〜</td>
