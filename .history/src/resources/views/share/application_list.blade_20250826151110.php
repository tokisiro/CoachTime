            ...
            <tr>
                    <td>
                        ...
                        @elseif($application->status === 'approved')
                            承認済み
                        @else
                            {{ $application->status }} {{-- その他のステータスがあれば表示 --}}
                        @endif
                    </td>
                    <td>{{ $application->user->name }}</td>
                    {{-- attendance が null でないことを確認する --}}
                    <td>{{ $application->attendance ? $application->attendance->date->format('Y-m-d') : 'N/A' }}</td>
                    ...
                    <td>
                        ...
                        @if($application->attendance)
                            <a class="application-list__parts-table--detail"
                               href="{{ route('attendance.show', ['id' => $application->attendance->id]) }}">詳細</a>
                        @else
                            N/A
                        @endif
                    </td>
