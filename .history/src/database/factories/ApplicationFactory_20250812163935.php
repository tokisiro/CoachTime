...
class ApplicationFactory extends Factory
{
    ...
    {
        return [
            'user_id' => User::all()->random()->id, // 既存のUserがいれば User::all()->random()->id でもOK
            'attendances_id' => Attendance::all()->random()->id, // 既存のAttendanceがいれば Attendance::all()->random()->id でもOK
            'status' => $this->faker->randomElement(['pending', 'approved', 'rejected']),
            'reason' => $this->faker->sentence(),
            'proposed_attendance' => $this->faker->time('H:i:s'),
