    ...
    public function configure()
    {
        ...
        return $this->afterCreating(function (Attendance $attendance) {
            ...
            // 勤務時間 (working_minutes) の計算
                ...
                // 休憩の生成
                // 休憩1（必ず生成）
                $break1Start = Carbon::parse($attendance->check_in_time)->addHours(4)->format('H:i:s');
                $break1End = Carbon::parse($break1Start)->addMinutes(60)->format('H:i:s'); // 60分休憩
                Break::factory()->create(['attendance_id' => $attendance->id,'start_time' => $break1Start,'end_time' => $break1End]);

                // 休憩2（50%の確率で生成）
                if ($this->faker->boolean(50)) {
                    $break2Start = Carbon::parse($break1End)->addHours(1)->format('H:i:s');
                    $break2End = Carbon::parse($break2Start)->addMinutes($this->faker->randomElement([15, 30]))->format('H:i:s'); // 15分または30分休憩
                    Break::factory()->create([
                ...
                // 休憩時間を取得して総勤務時間から減算
                $attendance->load('breaks'); // 休憩をロードし直す必要がある場合がある
