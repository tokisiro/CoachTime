...
class UserInformationAcquisitionTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->attendanceRecord1 = Attendance::factory()->for($this->employeeUser1, 'user') // Userリレーションを設定 (もし Attendance モデルに user() リレーションがあれば)
            ->hasBreaks(3, [->create([
        ...
        $this->break1ForAttendance1 = \App\Models\Breaks::factory()->create([
        ...
        \App\Models\Breaks::factory()->create([
            'attendance_id' => $this->attendanceRecord1->id,
            'start_time' => Carbon::parse('2025-10-01 14:00'),
            'end_time' =>  Carbon::parse('2025-10-01 15:00'), // +1時間
        ]);

        \App\Models\Breaks::factory()->create([
            'attendance_id' => $this->attendanceRecord1->id,
            'start_time' => Carbon::parse('2025-10-01 16:00'),
            'end_time' =>  Carbon::parse('2025-10-01 17:00'), // +1時間
        ]);
        ...
        dd([
            'attendanceRecord1_id' => $this->attendanceRecord1->id,
            'employeeUser1_id' => $this->employeeUser1->id,
            'attendanceRecord1_breaks_count' => $this->attendanceRecord1->breaks->count(),
            'attendanceRecord1_breaks' => $this->attendanceRecord1->breaks->toArray(),
        ]);
    ...
    public function admin_can_view_a_users_attendance_list()
    {
        ...
        $response = $this->get(route('admin.attendances.staff.list', ['user' => $this->employeeUser1->id,    'year' => Carbon::parse($this->attendanceRecord1->date)->year,
    ...
    public function admin_can_view_previous_months_attendance_data()
    {
        ...
        $lastMonthAttendance = Attendance::factory()->create([
            ...
            'check_in_time' => Carbon::today()->subMonth()->hour(9)->minute(0)->second(0),
            'check_out_time' => Carbon::today()->subMonth()->hour(18)->minute(0)->second(0),
        ]);

        // 1. 管理者ユーザーにログインをする
        $this->actingAs($this->adminUser, 'admin');

        // 2. 勤怠一覧ページを開く (現在月を表示するルートを想定)
        // 例: /admin/users/{user_id}/attendances?month=2023-10 のような形式
        $currentMonth = Carbon::today()->format('Y-m');
        $response = $this->get(route('admin.users.attendances.index', [
            'user' => $this->generalUser1->id,
            'month' => $currentMonth // 現在月を明示的に指定
        ]));
        $response->assertStatus(200);
        $response->assertSee(Carbon::today()->format('Y年m月')); // 現在月が表示されているか確認

        // 3. 「前月」ボタンを押す
        // 通常、「前月」ボタンは GET リクエストで `month` パラメータを前月に変更して送信します。
        ...
        // 前月の情報が表示されている
        $response->assertSee(Carbon::today()->subMonth()->format('Y年m月')); // 前月が表示されているか確認
        $response->assertSee(Carbon::parse($lastMonthAttendance->date)->format('Y/m/d')); // 前月の勤怠データが表示されているか
    ...
    public function admin_can_view_next_months_attendance_data()
    {
        ...
        $response->assertStatus(200);
        $response->assertSee(Carbon::today()->format('Y年m月'));
