...
class DetailAcquisitionFunctionTest extends TestCase
{
    ...
    public function DetailAcquisitionFunction()
    {
        ...
        $break1 = \App\Models\Breaks::factory()->create([
            ...
            'start_time' => Carbon::parse($attendance->start_time)->addHours(1)->toDateTimeString(), // 10:00
            'end_time' => Carbon::parse($attendance->end_time)->addHours(2)->toDateTimeString(),   // 11:00 (1時間休憩)
        ]);

        $break2 = \App\Models\Breaks::factory()->create([
            'attendance_id' => $attendance->id,
            'start_time' => Carbon::parse($attendance->start_time)->subHours(2)->toDateTimeString(),
            'end_time' => Carbon::parse($attendance->end_time)->subHours(1)->toDateTimeString(),
        ]);
        ...
        // 勤怠詳細画面に遷移したことを確認
        // 例: その日の日付や、出勤・退勤時刻が表示されていることを確認
        ...
        // 休憩1の時刻をアサート
        $break1StartTimeFormatted = Carbon::parse($break1->start_time)->format('H:i');
        $break1EndTimeFormatted = Carbon::parse($break1->end_time)->format('H:i');
        $response->assertSee('value="' . $break1StartTimeFormatted . '"', false);
        $response->assertSee('value="' . $break1EndTimeFormatted . '"', false);

        // 休憩2の時刻をアサート
        $break2StartTimeFormatted = Carbon::parse($break2->start_time)->format('H:i');
        $break2EndTimeFormatted = Carbon::parse($break2->end_time)->format('H:i');
        $response->assertSee('value="' . $break2StartTimeFormatted . '"', false);
        $response->assertSee('value="' . $break2EndTimeFormatted . '"', false);
    }
}
