...
class AttendanceDetailsCorrectionFunctionTest extends TestCase
{
    ...
    public function test_break_start_after_closing_time_shows_error()
    {
        ...
        $redirectResponse = $response->baseResponse;

        $errors = $response->session()->get('errors');

    if ($redirectResponse instanceof \Illuminate\Http\RedirectResponse) {
        $session = session(); // グローバルヘルパーの session() を使用してセッションストアにアクセス
        $errors = $session->get('errors');

        if ($errors instanceof \Illuminate\Support\MessageBag) {
            dd($errors->getMessages());
        } else {
            dd('Errors not found in global session or not a MessageBag instance.');
        }
    } else {
        dd('Response is not a RedirectResponse. Type: ' . get_class($redirectResponse));
    }
