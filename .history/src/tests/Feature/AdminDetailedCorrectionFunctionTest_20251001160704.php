...
class AdminDetailedCorrectionFunctionTest extends TestCase
{
    ...
    public function test_admin_can_approve_application_and_update_attendance()
    {
        ...
        $pendingApplication1 = Application::factory()->create([
            ...
            'attendance_id' => $this->attendanceRecord1->id,
            'status' => 'pending',
            'proposed_check_in_time' => '07:30:00', // 時刻フォーマットをHH:MM:SSに統一
            'proposed_closing_time' => '10:30:00', // 時刻フォーマットをHH:MM:SSに統一
            'proposed_remarks' => '幼稚園呼び出しにより早退希望', // ★コントローラに合わせて proposed_remarks を使用
            // 申請には reason もあるはずなので、ファクトリで設定されていなければ追加
            'reason' => '家族都合',
        ]);

        // 勤怠レコードに関連する既存の休憩レコードを作成 (これは変更前として扱われます)
        $existingBreak = Breaks::factory()->create([
            'attendance_id' => $this->attendanceRecord1->id,
            'start_time' => '12:00:00', // 変更前の休憩時間
            'end_time' => '13:00:00',   // 変更前の休憩時間
        ]);

        // ★申請に紐づく休憩レコードを作成し、提案時間を設定
        // コントローラがこの proposedBreak を更新することを期待
        $proposedBreakForApplication = Breaks::factory()->create([
            'attendance_id' => $this->attendanceRecord1->id, // 同じ勤怠レコードに紐づく
            'application_id' => $pendingApplication1->id,     // この申請に紐づく休憩
            'start_time' => '12:00:00', // 既存の休憩とは別のレコードで、元の値は適当で良い
            'end_time' => '13:00:00',   // 同上
            'proposed_start_time' => '09:30:00', // ★提案される休憩開始時刻
            'proposed_end_time' => '09:40:00',   // ★提案される休憩終了時刻
        ]);


        // 管理者としてログインし、承認処理を実行
        // コントローラは勤怠記録のIDを受け取るので、attendanceRecord1->id を使用
        // リクエストメソッドはコントローラが受け取れるもの（POST, PATCHなど）に合わせる必要があります
        // 今回の approve アクションは GET ではなくフォーム送信で呼ばれると仮定し、POST を使用
        $response = $this->actingAs($this->adminUser, 'admin')
                         ->post(route('admin.approve', $this->attendanceRecord1->id)); // ★ ルート名と引数を修正

        // リダイレクトされることを確認 (または成功ステータス)
        // コントローラは 'attendance.showDetails' にリダイレクトしているので、それをテスト
        ...
        $this->assertDatabaseHas('attendances', [
            ...
            'check_in_time' => '07:30:00', // 提案された時刻
            'closing_time' => '10:30:00', // 提案された時刻
            'remarks' => '幼稚園呼び出しにより早退希望', // 提案された備考
        ]);
        ...
        $this->assertDatabaseHas('breaks', [
            'id' => $existingBreak->id,
            'attendance_id' => $this->attendanceRecord1->id,
            'start_time' => '12:00:00', // 変更前と変わっていないことを期待
            'end_time' => '13:00:00',   // 変更前と変わっていないことを期待
            'application_id' => null, // 既存の休憩は application_id を持たない
        ]);
        ...
        $this->assertDatabaseHas('breaks', [
            ...
            'attendance_id' => $this->attendanceRecord1->id,
            'start_time' => '09:30:00', // ★提案された休憩開始時刻が適用されている
            'end_time' => '09:40:00',   // ★提案された休憩終了時刻が適用されている
            'proposed_start_time' => null, // ★クリアされていることを確認
            'proposed_end_time' => null,   // ★クリアされていることを確認
            'application_id' => null,      // ★クリアされていることを確認
        ]);
    }
    }
}
