...
class BreakFunctionTest extends TestCase
{
    ...
    public function test_multiple_breaks_can_be_taken_in_a_day()
    {
        ...
        $response->assertStatus(200);
        $response->assertSee('出勤中'); // 休憩から戻ったので「出勤中」に戻っているはず
        $response->assertSee('休憩入'); // 再度「休憩入」ボタンが表示されていること
        ...
        $this->assertDatabaseHas('break_times', [
            ...
            'break_start_time' => $breakStartTime1->toTimeString(),
            'break_end_time' => $breakEndTime1->toTimeString(),
        ]);
