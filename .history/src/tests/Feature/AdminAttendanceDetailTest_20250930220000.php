...
class AdminAttendanceDetailTest extends TestCase
{
    ...
    public function error_message_is_displayed_if_break_start_time_is_after_closing_time()
    {
        ...
        $errors = session()->get('errors')->getBag('default'); // Default エラーバッグを取得
if ($errors->has('breaks.0.start_time')) {
    // breaks.0.start_time のエラーメッセージを全てダンプ
    dd($errors->get('breaks.0.start_time'));
} else {
    // breaks.0.start_time のエラーがない場合、全てのエラーをダンプ
    dd($errors->toArray());
}
