...
class UserInformationAcquisitionTest extends TestCase
{
    ...
    public function admin_can_view_next_months_attendance_data()
    {
        ...
        $nextMonthAttendance = Attendance::factory()->create([
            ...
            'check_in_time' => Carbon::today()->addMonth()->hour(9)->minute(0),
            'closing_time' => Carbon::today()->addMonth()->hour(18)->minute(0),
        ]);
        ...
        // 「翌月」ボタンを押す
        $nextMonth = Carbon::today()->addMonth()->month;
        $response = $this->actingAs($this->adminUser, 'admin')->get(route('admin.attendances.staff.list', [
            'user' => $this->employeeUser1->id,
            'month' => $nextMonth
        ]));
        ...
        // 翌月の情報が表示されている
        $response->assertSee(Carbon::today()->addMonth()->format('Y年n月')); // 翌月が表示されているか確認
        $response->assertSee(Carbon::parse($nextMonthAttendance->date)->format('Y年n月')); // 翌月の勤怠データが表示されているか
        $response->assertDontSee(Carbon::parse($this->attendanceRecord1->date)->format('Y/m/d')); // 今月の勤怠データは表示されないか
    }
    ...
    public function admin_can_navigate_to_attendance_details_page()
    {
        ...
        $response = $this->actingAs($this->adminUser, 'admin')->get(route('admin.showDetails', ['id' => $this->attendanceRecord1->id]));

        $response->assertStatus(200);

        // 詳細ページに表示されるべき情報があることを確認
        $response->assertSee(Carbon::parse($this->attendanceRecord1->date)->format('Y年'));
        $response->assertSee(Carbon::parse($this->attendanceRecord1->date)->format('m月d日'));
        $response->assertSee(Carbon::parse($this->attendanceRecord1->check_in_time)->format('H:i'));
        $response->assertSee(Carbon::parse($this->attendanceRecord1->check_out_time)->format('H:i'));
        $response->assertSee('承認状態'); // または承認ステータスが表示されていることを確認
        $response->assertSee($this->employeeUser1->name . 'さんの勤怠詳細'); // ユーザー名が表示されているか
    }
}
