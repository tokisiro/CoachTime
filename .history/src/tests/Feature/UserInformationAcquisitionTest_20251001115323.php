...
class UserInformationAcquisitionTest extends TestCase
{
    ...
    public function admin_can_view_previous_months_attendance_data()
    {
        ...
        $response->assertStatus(200);
        $response->assertSee(Carbon::today()); // 現在月が表示されているか確認

        $previousYear = Carbon::today()->subYear()->year;
        $previousMonth = Carbon::today()->subMonth()->month->format('Y-m');

        //「前月」ボタンを押す
        $response = $this->actingAs($admin)->get(route('admin.attendances.staff.list', [
    'user' => $staff->id,
    'year' => $previousYear, // 前月の年
