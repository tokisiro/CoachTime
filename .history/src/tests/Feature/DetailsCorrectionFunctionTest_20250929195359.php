...
class DetailsCorrectionFunctionTest extends TestCase
{
    ...
    public function user_sees_all_their_approved_applications_on_list_page()
    {
        ...
        $applicationApproved->reviewed_at = Carbon::now();
        ...
        //申請一覧画面を開く
        $applicationsListPage = $this->get(route('showApplicationsList',['status' => 'approved']));
        $applicationsListPage->assertOk();

        // 管理者が承認した修正申請が全て表示されていることを確認
        $applicationsListPage->assertSee($approvedRemarks);
        $applicationsListPage->assertSee('承認済み');

        //申請一覧画面を開く
        $applicationsListPage = $this->get(route('showApplicationsList',['status' => 'pending']));
        $applicationsListPage->assertOk();
    ...
    public function clicking_details_button_navigates_to_application_details_page()
    {
        ...
        $this->post(route('applications', ['attendance_id' => $this->attendance->id]), [
        ...
        $applicationsListPage = $this->get(route('showApplicationsList', ['status' => 'pending']));
        $applicationsListPage->assertOk();
        $applicationsListPage->assertSee($remarks);

        // 「詳細」ボタンを押す
        // 仮のルート名: 'user.applications.show'
        $response = $this->get(route('attendance.showDetails', ['id' => $application->attendance->id]));

        // 期待挙動：勤怠詳細画面に遷移する
        $response->assertOk();
        $response->assertSee('勤怠詳細');
        $response->assertSee($remarks);
        $response->assertSee('出勤' . $proposedCheckInTime);
        $response->assertSee('退勤' . $proposedClosingTime);
        $response->assertSee('承認待ちのため修正できません'); // ステータスも表示されているか
    }
}
