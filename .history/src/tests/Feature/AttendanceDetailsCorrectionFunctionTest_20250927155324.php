...
class AttendanceDetailsCorrectionFunctionTest extends TestCase
{
    ...
    public function break_start_after_closing_time_shows_error()
    {
        ...
        // 2. 勤怠詳細ページを開く（このテストでは必須ではないが、フローに沿って記述）
        $this->actingAs($user)->get(route('attendance.showDetails', $attendance->id))
             ->assertStatus(200);

        // 休憩のIDを取得 (setUpAttendanceDataで作成した最初の休憩)
        $firstBreak = $attendance->breaks->first();

        // 3. 休憩開始時間を退勤時間より後に設定する
        //    (退勤時間: 17:00:00、休憩開始時間: 18:00:00)
        //    リクエストデータは DetailsRequest が期待する配列形式にする
        $response = $this->actingAs($user)->post(route('applications', $attendance->id), [
            'date' => $attendance->date,
            'check_in_time' => '09:00:00',
            'closing_time' => '17:00:00', // 退勤時間
            'breaks' => [
                [
                    'id' => $firstBreak->id, // 既存の休憩レコードのID
                    'start_time' => '18:00:00', // 問題のある休憩開始時間
                    'end_time' => '19:00:00',
                ],
                // 他の休憩も必要に応じて含める（ここでは最初の休憩のみで十分）
                [
                    'id' => $attendance->breaks[1]->id, // 2番目の休憩
                    'start_time' => '15:00:00',
                    'end_time' => '15:30:00',
                ],
            ],
        ...
        // 4. 保存処理をする -> $response に結果が格納される

        // 期待挙動: リダイレクトが行われることを確認
