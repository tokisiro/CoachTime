...
class UserInformationAcquisitionTest extends TestCase
{
    ...
    public function admin_can_view_a_users_attendance_list()
    {
        ...
        $response = $this->get(route('admin.attendances.staff.list', ['user' => $this->test1Attendance->id]));
        ...
        $response->assertSee(Carbon::parse($this->test1Attendance->date)->format('Y/m/d'));
        $response->assertSee(Carbon::parse($this->test1Attendance->)->format('H:i'));
        $response->assertSee(Carbon::parse($this->test1Attendance->check_out_time)->format('H:i'));

        // 休憩時間も表示されているか確認 (休憩時間がある場合)
        $firstBreak = $this->testAttendance->breaks->first();
        if ($firstBreak) {
            $response->assertSee(Carbon::parse($firstBreak->start_time)->format('H:i'));
            $response->assertSee(Carbon::parse($firstBreak->end_time)->format('H:i'));
        }
    }
}
