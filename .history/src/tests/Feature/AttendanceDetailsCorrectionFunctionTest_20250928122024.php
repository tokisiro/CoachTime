...
class AttendanceDetailsCorrectionFunctionTest extends TestCase
{
    ...
    //{
        ...
        //$finalResponse->assertSee('休憩時間が不適切な値です'); // ページ内にメッセージが表示されているか
    //}
    ...
    public function test_empty_note_shows_error()
    {
        ...
        $finalResponse->assertSee('備考を記入してください');
    }
    ...
    public function test_correction_request_is_created()
    {
        ...
        $this->actingAs($user)->patch(route('applications', $attendance->id), [
            'date' => $attendance->date,
        ...
        $response->assertRedirect(route('attendance.showDetails', $attendance->id));


        $response->assertSessionHas('* 承認待ちのため修正できません。');

        // データベースに修正申請が保存されたことを確認
        $this->assertDatabaseHas('applications', [
            'user_id' => $user->id,
            'attendance_id' => $attendance->id,
            'proposed_check_in_time' => $updatedCheckInTime,
            'proposed_closing_time' => $updatedClosingTime,
            'proposed_remarks' => $updatedNote,
        ...
        $adminApprovalResponse = $this->get(route('admin.approve')); // ルーティング名を適宜変更
        ...
        $adminRequestListResponse = $this->get(route('admin.request')); // ルーティング名を適宜変更
