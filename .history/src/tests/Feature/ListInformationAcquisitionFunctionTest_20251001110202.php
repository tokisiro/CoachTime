...
class ListInformationAcquisitionFunctionTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->otherUserAttendanceData = Attendance::factory()->create([
            ...
            'working_minutes' => $workingMinutesOther,
            'status' => 'submitted',
        ]);
    }
    ...
    public function testUserAttendanceIsDisplayed(): void
    {
        ...
        $response->assertSeeText(Carbon::parse($this->userAttendanceData1->date)->isoFormat('MM/DD(ddd)'));
        $response->assertSeeText(Carbon::parse($this->userAttendanceData1->check_in_time)->format('H:i'));
        $response->assertSeeText(Carbon::parse($this->userAttendanceData1->closing_time)->format('H:i'));
