...
class AdminAttendanceDetailTest extends TestCase
{
    ...
    public function error_message_is_displayed_if_break_start_time_is_after_closing_time()
    {
        ...
        $invalidData = [
            ...
            'break_end_time' => '20:00', // 休憩開始と合わせる
            'remarks' => 'テスト備考',
        ];

        $response = $this->actingAs($this->adminUser, 'admin')->put(route('admin.approve', ['id' => $this->testAttendance->id]), $invalidData);

        $response->assertSessionHasErrors(['breaks.*.start_time', 'closing_time']);
        $response->assertStatus(302);
