...
class AdminDetailedCorrectionFunctionTest extends TestCase
{
    ...
    public function test_admin_can_view_all_pending_applications()
    {
        ...
        $this->attendanceRecord = Attendance::factory()->create([
        ...
        $pendingApplication1 = Application::factory()->create([
            ...
            'proposed_closing_time' => '20:30:00',
            'reason' => '電車遅延',
        ]);
        $pendingApplication2 = Application::factory()->create([
            ...
            'attendance_id' => $this->attendanceRecord1->id,
            'status' => 'pending',
            'application_date' => Carbon::today()->subDays(3),
            'current_check_in_time' => '09:00:00',
            'proposed_check_in_time' => '09:15:00',
            'reason' => '体調不良',
        ]);
        ...
        $approvedApplication = Application::factory()->create([
            ...
            'status' => 'approved',
            'application_date' => Carbon::today()->subDays(1),
            'current_check_in_time' => '09:00:00',
            'proposed_check_in_time' => '09:00:00',
            'reason' => '承認済み',
        ]);

        // 管理者としてログインし、承認待ちの申請一覧ページにアクセス
        // 修正申請一覧のルートと、承認待ちを表示するためのクエリパラメータを調整してください。
        $response = $this->actingAs($this->admin)->get(route('admin.applications.index', ['status' => 'pending']));

        $response->assertStatus(200);

        // 承認待ちの申請が表示されていることを確認
        $response->assertSee($pendingApplication1->user->name);
        $response->assertSee($pendingApplication1->application_date->format('Y/m/d'));
        $response->assertSee('承認待ち');
        $response->assertSee($pendingApplication2->user->name);
        $response->assertSee($pendingApplication2->application_date->format('Y/m/d'));
        $response->assertSee('承認待ち');
    ...
    public function test_admin_can_view_all_approved_applications()
    {
        ...
        $approvedApplication1 = Application::factory()->create([
            'user_id' => $this->normalUser->id,
            'status' => 'approved',
            'application_date' => Carbon::today()->subDays(5),
            'current_check_in_time' => '09:00:00',
            'proposed_check_in_time' => '08:30:00',
            'reason' => '電車遅延',
        ]);
        ...
        $response = $this->actingAs($this->admin)->get(route('admin.applications.index', ['status' => 'approved']));

        $response->assertStatus(200);

        // 承認済みの申請が表示されていることを確認
        $response->assertSee($approvedApplication1->user->name);
        $response->assertSee($approvedApplication1->application_date->format('Y/m/d'));
        $response->assertSee('承認済み');
        $response->assertSee($approvedApplication2->user->name);
        $response->assertSee($approvedApplication2->application_date->format('Y/m/d'));
        $response->assertSee('承認済み');
