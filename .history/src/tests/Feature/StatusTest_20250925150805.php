...
class StatusTest extends TestCase
{
    ...
    {
        // テストの日時を固定
        $testDateTime = Carbon::create(2023, 10, 27, 18, 0, 0);
        Carbon::setTestNow($testDateTime);

        // 1. ステータスが退勤済のユーザーにログインする
        // (今日の勤怠記録があり、closing_timeが設定されている)
        $user = User::factory()->create();
        $this->actingAs($user);
        ...
        $response->assertStatus(200);
        $response->assertSee('退勤済'); // 期待挙動: 画面上に「退勤済」と表示される

        // 退勤済みなので、すべての打刻ボタンが非表示になっていることを確認
        $response->assertDontSee('出勤');
        $response->assertDontSee('退勤');
        $response->assertDontSee('休憩');
        $response->assertDontSee('休憩終了');
    }
