...
class AdminAttendanceListTest extends TestCase
{
    ...
    public function admin_can_see_all_users_attendance_for_the_current_day()
    {
        ...
        $response = $this->actingAs($this->adminUser,)->get(route('admin.attendances.list'));
    ...
    {
        $today = Carbon::today()->format('Y年m月d日'); // 画面表示される日付フォーマットに合わせる (例: 2023年01月01日)

        // 管理者としてログインし、勤怠一覧画面にアクセス
        $response = $this->actingAs($this->adminUser)->get(route('admin.attendance.list'));
    ...
    public function admin_can_navigate_to_previous_day_attendance()
    {
        $today = Carbon::today();
        $yesterday = Carbon::yesterday();
        $formattedYesterday = $yesterday->format('Y年m月d日'); // 画面表示される日付フォーマットに合わせる

        // 昨日の勤怠データを作成
        $user1 = User::factory()->create(['name' => '前日ユーザー', 'is_admin' => false]);
        Attendance::factory()->create([
            'user_id' => $user1->id,
            'date' => $yesterday,
        ...
        $user2 = User::factory()->create(['name' => '今日ユーザー', 'is_admin' => false]);
        Attendance::factory()->create([
            'user_id' => $user2->id,
            'date' => $today,
            'check_in_time' => '09:00:00',
        ...
        $response = $this->actingAs($this->adminUser)->get(route('admin.attendance.list', ['date' => $yesterday->format('Y-m-d')]));

        $response->assertOk(); // HTTPステータスコード200を確認

        // 期待挙動: 前日の日付の勤怠情報が表示される
        $response->assertSee($formattedYesterday); // 前日の日付が表示されていることを確認
        $response->assertSee($user1->name);
        $response->assertSee('08:30');
        $response->assertSee('17:30');
        $response->assertSee('前日の勤怠データ');

        // 今日の勤怠は表示されないことを確認
        $response->assertDontSee($user2->name);
        $response->assertDontSee('今日の勤怠データ');
    }
    ...
    public function admin_can_navigate_to_next_day_attendance()
    {
        ...
        $formattedTomorrow = $tomorrow->format('Y年m月d日'); // 画面表示される日付フォーマットに合わせる

        // 翌日の勤怠データを作成
        $user1 = User::factory()->create(['name' => '翌日ユーザー', 'is_admin' => false]);
        Attendance::factory()->create([
            'user_id' => $user1->id,
            'date' => $tomorrow,
        ...
        // 通常は /admin/attendance/list?date=YYYY-MM-DD のようなURLになる
        $response = $this->actingAs($this->adminUser)->get(route('admin.attendance.list', ['date' => $tomorrow->format('Y-m-d')]));
