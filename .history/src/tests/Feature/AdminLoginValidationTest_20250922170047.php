...
class AdminLoginValidationTest extends TestCase
{
    ...
    {
        $this->createAdminUser(); // 管理者ユーザーを作成

        $loginData = [
            'email' => 'wrong_admin@example.com', // 存在しないメールアドレス
            'password' => 'adminpassword',
        ];

        $response = $this->post('/admin/login', $loginData);

        // FortifyServiceProviderの authenticateUsing クロージャ内でValidationExceptionをスローしている場合
