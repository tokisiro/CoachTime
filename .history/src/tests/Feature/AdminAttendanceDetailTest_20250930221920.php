...
class AdminAttendanceDetailTest extends TestCase
{
    ...
    public function error_message_is_displayed_if_break_start_time_is_after_closing_time()
    {
        ...
        $invalidData = [
            ...
            'breaks' => [
    [
        'id' => $this->testAttendance->id,
        'start_time' => '19:00', // 退勤時間より後の休憩開始時間
        'end_time' => '20:00',
    ],
],
        ];
        ...
        if ($response->status() === 500) {
            ...
            echo $response->exception->getFile() . " line " . $response->exception->getLine() . "\n";
            // バックトレースも必要なら
            // echo $response->exception->getTraceAsString() . "\n";
            echo "-----------------------------------------\n";
            $this->fail("500 Internal Server Error occurred. See details above."); // テストを強制的に失敗させる
        }
        $response->assertStatus(302);
