...
class UserInformationAcquisitionTest extends TestCase
{
    ...
    public function admin_can_view_a_users_attendance_list()
    {
        ...
        // 2. 選択したユーザーの勤怠一覧ページを開く (例: /admin/users/{user_id}/attendances)
        // ルート名が 'admin.users.attendances.index' のような形を想定
        $response = $this->get(route('admin.users.attendances.index', ['user' => $this->generalUser1->id]));
        ...
        $response->assertSee(Carbon::parse($this->testAttendance->date)->format('Y/m/d'));
