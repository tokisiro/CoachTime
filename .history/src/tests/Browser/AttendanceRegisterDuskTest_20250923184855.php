...
class AttendanceRegisterDuskTest extends DuskTestCase
{
    ...
    public function test_current_datetime_is_displayed_correctly_on_attendance_page_with_javascript(): void
    {
        ...
        $this->browse(function (Browser $browser) use ($user, $password) {
            // Step 1: ログインページにアクセスし、手動でログインを行う
            $browser->visit('/login') // ログインページにアクセス
                    ...
                    ->assertRouteIs('attendance.register') // 勤怠打刻画面にリダイレクトされたことを確認
                    ->assertSee('勤務外') // ページ内の特定のテキストを確認
                    ->screenshot('manual-login-and-attendance-page'); // ログイン後のスクリーンショット

            // Step 2: 日付と時刻の表示を検証
            // PHPがテストを実行した時点の時刻を取得
            $phpNow = Carbon::now();

            // 日付部分の期待値 (例: 2023年10月26日(木))
            ...
            $this->assertTrue($isTimeMatch, "時刻が正しく表示されていません。期待値: {$expectedTimeMinute} または {$phpNow->addMinute()->format('H:i')}, UI: {$uiTime}");
        });
    }
}
