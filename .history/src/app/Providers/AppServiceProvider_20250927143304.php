...
class AppServiceProvider extends ServiceProvider
{
    ...
    Validator::extend('break_start_time_valid', function ($attribute, $value, $parameters, $validator) {
        ...
        'attribute' => $attribute,
        'value' => $value, // 休憩開始時間
        'closingTime' => $closingTime, // リクエストの退勤時間
        'parsed_closing' => $closing, // パース後の退勤時間
        'breakStartTime' => $breakStartTime, // パース後の休憩開始時間
        'condition_check_in' => ($checkIn && $breakStartTime->lt($checkIn)), // 出勤時間より前
        'condition_closing' => ($closing && $breakStartTime->gt($closing)), // 退勤時間より後
        'condition_break_end' => ($breakEndTime && $breakStartTime->gte($breakEndTime)), // 休憩終了時間以上
    ]);
        ...
        if ($closing && $breakStartTime->gt($closing)) {
            dd('Error: breakStartTime is after closing');
            return false;
        }
