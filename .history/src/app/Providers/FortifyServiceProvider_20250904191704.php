...
class FortifyServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        if (Auth::guard('admin')->check()) { // admin guard でログインしているか確認
            Auth::guard('admin')->logout(); // admin guard でログアウト
            return redirect('/admin/login'); // admin のログインページへリダイレクト
        }

        Auth::guard('web')->logout(); // web guard でログアウト
        return redirect('/login'); // 通常のトップページへリダイレクト
        });
    }
}