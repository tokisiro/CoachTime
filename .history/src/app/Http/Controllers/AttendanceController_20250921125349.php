...
//勤怠詳細画面関連
    ...
    public function showDetails(Request $request, $id)
    {
        ...
        $pendingProposedBreaks = [];

        if ($latestApprovedApplication) {
            $decodedBreaks = json_decode($latestApprovedApplication->proposed_breaks, true) ?? [];
            $existingBreakIds = $attendance->breaks->pluck('id')->toArray();
            $approvedProposedBreaks = collect($decodedBreaks)->filter(function ($break) use ($existingBreakIds) {
        ...
        foreach ($pendingProposedBreaks as $index => $break) {
            $newBreakStartTimes['pending_' . $index] = $this->formatTime($break['start_time'] ?? null);
            $newBreakEndTimes['pending_' . $index] = $this->formatTime($break['end_time'] ?? null);
        }

        // latestApprovedApplication からのデータ
        foreach ($approvedProposedBreaks as $index => $break) {
            $newBreakStartTimes['approved_' . $index] = $this->formatTime($break['start_time'] ?? null);
            $newBreakEndTimes['approved_' . $index] = $this->formatTime($break['end_time'] ?? null);
        }

        // Bladeで使うための最終的な値
        $bladeNewBreakEndTimes = [];
        for ($i = 0; $i < count($pendingProposedBreaks) || $i < count($approvedProposedBreaks); $i++) {
            $value = '';
            if ($pendingApplication && isset($newBreakEndTimes['pending_' . $i])) {
                $value = $newBreakEndTimes['pending_' . $i];
            } elseif ($latestApprovedApplication && isset($newBreakEndTimes['approved_' . $i])) {
                $value = $newBreakEndTimes['approved_' . $i];
            }
            $bladeNewBreakEndTimes[$i] = $value;
        }

        $isPendingApproval = (bool) ($attendance->applications->where('status', 'pending')->sortByDesc('created_at')->first());

        // 勤怠記録が最終的に確定済みであるかどうか
        $isAttendanceFinalized = ($attendance->status === 'approved' || (bool) ($attendance->applications()->where('status', 'approved')->first()));
    ...
    private function formatTime(?string $timeStr): string
    {
        ...
        try {
            ...
            // パースエラーの場合は空文字列を返す
            \Log::warning("Failed to parse time string '{$timeStr}': " . $e->getMessage());
            return '';
        }
    }
