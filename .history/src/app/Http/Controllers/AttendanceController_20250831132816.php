...
//勤怠詳細画面関連
    ...
    public function showDetails(Request $request, $id)
    {
        ...
        // まず管理者ガードでログインしているか確認
        if (Auth::guard('admin')->check()) {
            $loggedInUser = Auth::guard('admin')->user();
        }
        // 管理者ガードでログインしていなければ、webガードでログインしているか確認
        // ここでの 'web' は、一般ユーザーがログインするデフォルトのガード名
        elseif (Auth::guard('web')->check()) {
            $loggedInUser = Auth::guard('web')->user();
        }

        // ログインしていない場合はエラー（またはログインページにリダイレクト）
        if (!$loggedInUser) {
            // 例: ログインページにリダイレクト
            return redirect()->route('login'); // あなたのログインルート名に合わせてください
            // または、シンプルに403 Forbiddenエラー
            // abort(403, 'ログインが必要です。');
        }
        ...
        // ロールに基づいてフィルタリング
        if ($loggedInUser->isGeneral()) { // 一般ユーザーの場合
        ...
        if (!$attendance) {
            // 404 Not Found (見つからない) または 403 Forbidden (アクセス禁止)
            // どちらが適切かはアプリケーションの要件によります。
            // 他人の勤怠を見ようとした場合は403、存在しない勤怠IDの場合は404が良いでしょう。
            // 今回はシンプルに404で統一
        ...
        // ビューは一般ユーザー用と管理者用で分けるか、一つのビューで表示を切り替えるか検討
        // 今回は normal.attendance_details を使用する前提
    ...
    public function showApplicationsList(Request $request)
    {
        ...
        return view('share.application_list', compact('applications', 'status'));
    }
...
public function showAdminDetails($id)
    {
        // 勤怠詳細表示ロジック (既存のコードに合わせてください)
        $attendance = Attendance::with('user', 'breaks', 'applications')->findOrFail($id);
