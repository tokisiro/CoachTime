...
//勤怠詳細画面関連
    ...
    public function applyEdit(Request $request, $id)
    {
        ...
        try {
            ...
            // Breaks テーブルへの保存 (申請された休憩時間)
            if ($request->has('breaks')) {
                foreach ($request->input('breaks') as $breakData) {
                    // IDがフォームから送られてくることを想定
                    // Form Request でIDが必須かつ数値であることなどをバリデーションできる
                    if (isset($breakData['id'])) {
                        // 該当の休憩レコードを取得
                        // Form Request で attendance_id と break_id の紐付けを検証することも可能
                        $breakRecord = Breaks::find($breakData['id']);

                        // 取得した休憩レコードが存在し、かつ現在の勤怠に紐付いていることを確認
                        // このチェックもForm Requestのauthorize()やrules()でより強固にできる
                        if ($breakRecord && $breakRecord->attendance_id === $attendance->id) {
                            $breakRecord->status = 'pending'; // ステータスを保留中に更新
                            $breakRecord->save();
                            // もし、申請後の休憩時間（proposed_start_time, proposed_end_time）も保持したい場合、
                            // Applicationモデルに関連テーブルを作成するか、Breaksモデルにカラムを追加することを検討
                        }
                    }
                }
            }
