...
//勤怠詳細画面関連
    ...
    public function showByDate(Request $request, $id)
    {
        ...
        // 勤怠に紐づく申請があり、そのステータスが 'pending' のものがあるかチェック
        $isPendingApproval = $attendance->applications->contains('status', 'pending');

        // もし、保留中の申請がある場合、その申請で追加・削除された休憩を考慮してbreaksコレクションをフィルタリング
        if ($isPendingApproval) {
            $pendingApplication = $attendance->applications
                ->where('status', 'pending')
                ->sortByDesc('created_at') // 最新の申請を取得
                ->first();

            if ($pendingApplication) {
                // breaksリレーションをフィルタリング
                $filteredBreaks = $attendance->breaks->filter(function ($break) use ($pendingApplication) {
                    // pending状態のbreaksレコードで、提案された開始・終了時刻が両方とも空のものは表示しない（削除を意図）
                    $correspondingPendingBreak = Breaks::where('attendance_id', $break->attendance_id)
                        ->where('application_id', $pendingApplication->id)
                        ->where('id', $break->id) // 既存の休憩の修正を特定
                        ->where('status', 'pending')
                        ->first();

                    if ($correspondingPendingBreak && empty($correspondingPendingBreak->proposed_start_time) && empty($correspondingPendingBreak->proposed_end_time)) {
                        return false; // 提案で削除される休憩は表示しない
                    }
                    return true; // それ以外の休憩は表示
                });

                // 新規追加される休憩（application_idがあるもの）もbreaksコレクションに含める
                $newPendingBreaks = Breaks::where('attendance_id', $attendance->id)
                    ->where('application_id', $pendingApplication->id)
                    ->whereNull('id') // IDがnullのものが新規追加された休憩
                    ->where('status', 'pending')
                    ->get();

                $attendance->setRelation('breaks', $filteredBreaks->merge($newPendingBreaks));
            }
        }
