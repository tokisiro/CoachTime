...
//出勤登録画面関連
    ...
    public function showAttendanceRegister()
    {
        ...
        // 現在の日時情報を取得し、ビューに渡す
        $now = Carbon::now();
        $dateStr = $now->isoFormat('YYYY年MM月DD日(ddd)'); // 例: 2023年10月27日(金)
    ...
    public function recordClosingTime(Request $request)
    {
        ...
        $currentDate = Carbon::today();
            ...
            $attendanceRecord = Attendance::where('user_id', $userId)
                ...
                ->whereNull('closing_time') // まだ退勤していない記録
                ->first();
    ...
    public function recordBreakIn(Request $request)
    {
        ...
        // 今日の出勤レコードを取得または作成
        $attendance = Attendance::where('user_id' , $userId) ->whereDate('date', $currentDate)
                                ->whereNotNull('check_in_time') // 出勤済みであること
                                ->whereNull('closing_time') // 退勤していないこと
                                ->first();
        // 出勤記録がない場合はエラー
        if (!$attendance) {
