...
//勤怠詳細画面関連
    ...
    public function showStaffAttendance(Request $request, $user)
    {
        ...
        // 表示する年月を決定
        // type="month" から 'YYYY-MM' 形式で値が来ることを想定
        $selectedMonth = $request->query('month_selector'); // type="month" の input の name 属性に合わせる
        ...
        $attendances = Attendance::where('users_id', $staff->id)
            ->whereBetween('date', [$startDate, $endDate])
            ->with('breaks') // 休憩時間をロード
            ->orderBy('date', 'asc')
            ->get();

        // 各勤怠レコードに休憩合計時間と勤務合計時間を追加（コントローラ側で処理）
        foreach ($attendances as $attendance) {
            $totalBreakMinutes = 0;
            if ($attendance->breaks->isNotEmpty()) { // 休憩データがある場合のみ計算
                foreach ($attendance->breaks as $break) {
                    if ($break->start_time && $break->end_time) {
                        $breakStart = Carbon::parse($break->start_time);
                        $breakEnd = Carbon::parse($break->end_time);
                        // 休憩終了時刻が休憩開始時刻より後の場合のみ加算
            ...
            // 勤怠レコードに休憩合計時間（分）とフォーマット済みの休憩時間を追加
            $attendance->formatted_total_break_time = sprintf('%02d:%02d', floor($totalBreakMinutes / 60), $totalBreakMinutes % 60);

            $attendance->working_minutes = ''; // デフォルト値
            if ($attendance->check_in_time && $attendance->closing_time) {
                $checkIn = Carbon::parse($attendance->check_in_time);
                $closing = Carbon::parse($attendance->closing_time);

                // 退勤時刻が出勤時刻より後の場合のみ計算
                if ($closing->greaterThan($checkIn)) {
                    $workingMinutes = $closing->diffInMinutes($checkIn) - $totalBreakMinutes;
                    // 勤務時間がマイナスにならないようにする
                    $attendance->working_minutes = sprintf('%02d:%02d', floor(max(0, $workingMinutes) / 60), max(0, $workingMinutes) % 60);
                }
            }
        }

        // 月の前後移動のためのURL
        // type="month" の場合、フォーム送信で処理するため、個別の前後月リンクは不要になることが多いですが、
        // もし必要なら以下のように生成します。
        ...
        // type="month" の input に設定するための現在選択されている年月文字列
        $currentMonthValue = $currentMonthCarbon->format('Y-m');

        // prevMonthUrl と nextMonthUrl は、もし前後月ボタンを残すなら使う
        // 例えば、フォームの hidden input に nextMonthCarbon->format('Y-m') を設定してsubmitする形など
        $prevMonthUrl = route('admin.attendances.staff.list', [
            'user' => $staff->id,
            'month_selector' => $prevMonthCarbon->format('Y-m')
        ]);
        $nextMonthUrl = route('admin.attendances.staff.list', [
            'user_id' => $staff->id,
            'month_selector' => $nextMonthCarbon->format('Y-m')
        ]);
        ...
        return view('admin.staff_attendance_list', compact(
            ...
            'year', // 現在の表示年
            'month', // 現在の表示月
            'currentMonthValue', // type="month" の input の value に設定する 'YYYY-MM' 形式
            'prevMonthUrl', // 前月リンク（必要であれば）
            'nextMonthUrl' // 翌月リンク（必要であれば）
        ));
    }
