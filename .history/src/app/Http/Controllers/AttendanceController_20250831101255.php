...
//勤怠詳細画面関連
    ...
    public function showAdminAttendanceList(Request $request){
        ...
        $startDateOfMonth = Carbon::parse($selectedMonth)->startOfMonth();
        $endDateOfMonth = Carbon::parse($selectedMonth)->endOfMonth();

        // 当月の日付の配列を生成
        $datesInMonth = [];
        for ($date = clone $startDateOfMonth; $date->lte($endDateOfMonth); $date->addDay()) {
        ...
        foreach ($datesInMonth as $date) {
            ...
            $attendancesByDate[$date] = $attendances;
        }

        return view('admin.admin_attendance_list', [
            'attendancesByDate' => $attendancesByDate, // 日付ごとの勤怠データを渡す
            'selectedMonth' => $selectedMonth,
        ]);
}

public function showdminDetails($id)
    {
