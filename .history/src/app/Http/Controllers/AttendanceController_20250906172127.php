...
//勤怠詳細画面関連
    ...
    public function showDetails(Request $request, $id)
    {
        ...
        if ($isAdminLoggedIn) {
            ...
            return view('admin.admin_attendance_details', compact('attendance', 'isPendingApproval', 'isAdminLoggedIn'));
        } else {
            // 一般ユーザーとしてログインしている場合は、一般ユーザー用の詳細ビューを返す
            return view('normal.attendance_details', compact('attendance', 'isPendingApproval', 'isAdminLoggedIn'));
        }
    ...
    public function showApplicationsList(Request $request)
    {
        ...
        if ($isAdminLoggedIn) {
            ...
            return view('admin.admin_application_list', compact('applications', 'status','isAdminLoggedIn'));
        } else {
    ...
    public function approve(Request $request, $id)
    {
        ...
        try {
            // 該当の勤怠記録と承認待ちの申請を取得
            $attendance = Attendance::with(['applications' => function($query) {
                $query->where('status', 'pending');
            }, 'breaks'])->find($id);
            ...
            // 承認待ちの申請が存在しない場合
            if ($attendance->applications->isEmpty()) {
                DB::rollback();
                return redirect()->back()->with('error', '承認待ちの申請がありません。');
            }
            ...
            foreach ($attendance->breaks as $break) {
                if ($break->application_id === $application->id && $break->status === 'pending') {
                    // この申請によって追加または変更された休憩であれば更新
                    $break->start_time = $break->proposed_start_time;
                    $break->end_time = $break->proposed_end_time;
                    $break->status = 'approved'; // 休憩のステータスも承認済みに変更
                    $break->save();
                }
            }
            ...
            return redirect()->route('admin.attendance.details', ['id' => $attendance->id])->with('success', '勤怠申請を承認しました。'); // 管理者用の詳細ページにリダイレクト
        } catch (\Exception $e) {
            DB::rollback();
            \Log::error('勤怠承認エラー: ' . $e->getMessage());
            return redirect()->back()->with('error', '勤怠承認中にエラーが発生しました。');
        }
    }
