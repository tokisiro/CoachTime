...
//勤怠詳細画面関連
    ...
    // 
    public function applyAttendanceEdit(Request $request, $id)
    {
        // 1. バリデーション
        $request->validate([
            'proposed_check_in_time' => 'nullable|date_format:H:i',
            'proposed_closing_time' => 'nullable|date_format:H:i|after:proposed_check_in_time',
            'proposed_break_start_time_1' => 'nullable|date_format:H:i',
            'proposed_break_end_time_1' => 'nullable|date_format:H:i|after:proposed_break_start_time_1',
            'proposed_break_start_time_2' => 'nullable|date_format:H:i',
            'proposed_break_end_time_2' => 'nullable|date_format:H:i|after:proposed_break_start_time_2',
            'proposed_remarks' => 'nullable|string|max:255',
            'reason' => 'required|string|max:500', // 申請理由を必須に
        ], [
            'proposed_closing_time.after' => '退勤時間は出勤時間より後に設定してください。',
            'proposed_break_end_time_1.after' => '休憩終了時間は開始時間より後に設定してください。',
            'proposed_break_end_time_2.after' => '休憩終了時間は開始時間より後に設定してください。',
        ...
        // 3. Applicationテーブルに申請レコードを作成
        $application = new Application();
        $application->user_id = Auth::id(); // 現在の認証ユーザーID
        $application->attendance_id = $attendance->id;
        $application->status = 'pending'; // 申請ステータスを「保留中」に設定
        $application->reason = $request->input('reason');

        // 修正後の出退勤時刻を保存 (NULLable なので空でもOK)
        $application->proposed_check_in_time = $request->input('proposed_check_in_time');
        $application->proposed_closing_time = $request->input('proposed_closing_time');

        // 修正後の備考を保存
        $application->proposed_remarks = $request->input('proposed_remarks');

        // 休憩時間の変更はremarksに含めるか、Applicationモデルに新しいカラムを追加する必要がある
        // 今回はシンプルに備考欄に追記する例
        $proposedBreaks = [];
        if ($request->filled('proposed_break_start_time_1') && $request->filled('proposed_break_end_time_1')) {
            $proposedBreaks[] = '休憩1: ' . $request->input('proposed_break_start_time_1') . '〜' . $request->input('proposed_break_end_time_1');
        }
        if ($request->filled('proposed_break_start_time_2') && $request->filled('proposed_break_end_time_2')) {
            $proposedBreaks[] = '休憩2: ' . $request->input('proposed_break_start_time_2') . '〜' . $request->input('proposed_break_end_time_2');
        }

        if (!empty($proposedBreaks)) {
            $application->proposed_remarks .= "\n[休憩修正] " . implode(', ', $proposedBreaks);
        }


        $application->save();

        // 4. ユーザーへのフィードバックとリダイレクト
        return redirect()->route('attendance.show', ['id' => $attendance->id])->with('success', '勤怠修正申請を受け付けました。');
        // または、勤怠一覧ページにリダイレクト
        // return redirect()->route('attendances.list')->with('success', '勤怠修正申請を受け付けました。');
    }
