...
//勤怠詳細画面関連
    ...
    public function showDetails(Request $request, $id)
    {
        ...
        $isAttendanceApproved = ($attendance->status === 'approved' || $attendance->status === 'finalized');

        // ビューに渡す変数
        $data = compact('attendance', 'isPendingApproval', 'isAdminLoggedIn', 'isAttendanceApproved');

        if ($isAdminLoggedIn) {
            // 管理者としてログインしている場合は、管理者用の詳細ビューを返す
            return view('admin.admin_attendance_details', compact('attendance', 'isPendingApproval', 'isAdminLoggedIn', 'isApproved'));
        } else {
            // 一般ユーザーとしてログインしている場合は、一般ユーザー用の詳細ビューを返す
            return view('normal.attendance_details', compact('attendance', 'isPendingApproval', 'isAdminLoggedIn', 'isApproved'));
        }
    ...
    public function approve(Request $request, $id)
    {
        ...
        try {
            ...
            // 最初の承認待ちの申請を取得（通常、1つの勤怠に対して1つの申請がpendingになることを想定）
        ...
        // 承認処理が行われた場合にクエリパラメータを追加
            if ($isApprovedAction) {
                return redirect()->route('admin.attendance.details', ['id' => $attendance->id, 'approved' => true]);
            } else {
                return redirect()->route('admin.attendance.details', ['id' => $attendance->id]);
            }
    } catch (\Exception $e) {
        DB::rollback();
        \Log::error('勤怠承認/更新エラー: ' . $e->getMessage());
        return redirect()->back()->with('error', '勤怠データの処理中にエラーが発生しました。');
    }
}
