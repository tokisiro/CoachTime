...
//勤怠詳細画面関連
    ...
    {
        // ユーザー認証チェック（一般ユーザーと管理者）
        $user = Auth::user();
        $admin = Auth::guard('admin')->user();

        if (!$user && !$admin) {
            return redirect()->route('login'); // ログインしていない場合はログインページへ
        }

        // Attendance とその関連モデル（user, breaks, applications）をロード
        $attendance = Attendance::with(['user', 'breaks', 'applications'])->find($id);

        if (!$attendance) {
            return redirect()->back()->with('error', '指定された勤怠記録が見つかりませんでした。');
        }

        // 一般ユーザーの場合、自分の勤怠のみ閲覧可能
        if ($user && $attendance->user_id !== $user->id) {
            return redirect()->back()->with('error', '閲覧権限がありません。');
        }
        ...
        // 承認済みの勤怠を判定するロジック
        // 1. 勤怠自体が直接承認済みになっている場合 (古いロジックや直接更新された場合)
        if ($attendance->status === 'approved') { // attendanceテーブルにstatusカラムがあり、それが'approved'の場合
            $isAttendanceApproved = true;
        }
        // 2. この勤怠に関連する承認済みの申請がある場合 (新しいロジック)
        $approvedApplication = $attendance->applications->where('status', 'approved')->first(); // 複数ある場合はどれか一つでもあれば
        if ($approvedApplication) {
            $isAttendanceApproved = true;
        }

        // 注意: もし「承認済み」の優先度が「承認待ち」より高い場合は、
        // 承認済みの判断の後に $isPendingApproval を上書きするロジックも検討
        // 例: if ($isAttendanceApproved) { $isPendingApproval = false; }


        return view('admin.admin_attendance_details', compact(
            'attendance',
            'isPendingApproval',
            'isAttendanceApproved',
            'pendingApplication', // ビューで利用するために渡す（承認待ち申請の内容を表示する場合）
            'approvedApplication' // ビューで利用するために渡す（承認済み申請の内容を表示する場合）
        ));
    }
