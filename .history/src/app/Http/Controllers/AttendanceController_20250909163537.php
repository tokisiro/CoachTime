...
//勤怠詳細画面関連
    ...
    public function showDetails(Request $request, $id)
    {
        ...
        // 管理者の場合は、user_idによるフィルタリングは不要なので、何も追加しない

        // 関連する勤怠情報を取得
        $attendance = $query->with(['user', 'breaks', 'applications'])->first();

        // 勤怠記録が見つからない、またはアクセス権限がない場合
        if (!$attendance) {
            abort(404, '指定された勤怠記録が見つからないか、アクセス権限がありません。');
        }

        // 承認待ちの申請があるかどうかのチェック
        $pendingApplication = $attendance->applications->where('status', 'pending')->first();
        $isPendingApproval = (bool) $pendingApplication;

        // 勤怠記録が承認済み（確定済み）であるかどうかのチェック
        $approvedApplication = $attendance->applications->where('status', 'approved')->first();
        $isAttendanceApproved = ($attendance->status === 'approved' || (bool) $approvedApplication);

        // クエリパラメータ'approved'がtrueの場合は、一旦強制的に承認済みとして扱う
        // これはリダイレクト直後の表示制御のため
        if ($request->query('approved') === 'true') {
            $isAttendanceApproved = true;
            // この場合、もし承認済み申請がないなら、attendanceの最新のデータをapprovedApplicationとして扱う
            if (!$approvedApplication && !$isPendingApproval) {
                $approvedApplication = new Application([
                    'proposed_check_in_time' => $attendance->check_in_time,
                    'proposed_closing_time' => $attendance->closing_time,
                    'proposed_remarks' => $attendance->remarks,
                    'proposed_breaks' => json_encode($attendance->breaks->map(function($break) {
                        return ['id' => $break->id, 'start_time' => $break->start_time, 'end_time' => $break->end_time];
                    })->toArray()),
                    'status' => 'approved',
                ]);
            }
        ...
        // 承認待ちが優先されるべきであれば、承認済み状態をリセット
        // （ただし、今回は承認済みであれば入力不可にするので、このままでOK）
        ...
        $data = compact(
            ...
            'isAdminLoggedIn',
            'pendingApplication',  // 承認待ちのApplicationモデルインスタンスを渡す
            'approvedApplication'  // 承認済みのApplicationモデルインスタンスを渡す
        );
    ...
    public function applications(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('attendance.showDetails', ['id' => $attendance->id])->with('success', '修正申請が正常に送信されました。');
    ...
    public function approve(Request $request, $id)
    {
        ...
        DB::beginTransaction();
            ...
            foreach ($attendance->breaks as $break) {
                $proposedBreakData = collect($application->proposed_breaks ?? [])->firstWhere('id', $break->id);
                    if ($proposedBreakData) {
                        $break->start_time = $proposedBreakData['start_time'];
                        $break->end_time = $proposedBreakData['end_time'];
                        $break->save();
                    }
                }
            ...
            if ($request->has('breaks')) {
                foreach ($request->input('breaks') as $BreakData) {
                $proposedBreaks[] = [
                'id' => $breakData['id'] ?? null, // 既存休憩のIDがあれば含める
        ...
        foreach ($request->input('new_breaks') as $newBreakData) {
            if (!empty($newBreakData['start_time']) || !empty($newBreakData['end_time'])) {
                $proposedBreaks[] = [
                    'id' => null, // 新規休憩なのでIDはなし
                    'start_time' => $newBreakData['start_time'] ?? null,
                    'end_time' => $newBreakData['end_time'] ?? null,
                ];
            }
        }
    }

            \App\Models\Application::create([ // App\Models\Application をuseするのを忘れずに
                'user_id' => $attendance->user_id,
                'attendance_id' => $attendance->id,
                'proposed_check_in_time' => $request->input('check_in_time'),
                'proposed_closing_time' => $request->input('closing_time'),
                'proposed_remarks' => $request->input('remarks'),
                'proposed_breaks' => json_encode($proposedBreaks), // 既存の休憩データをJSON形式で保存
                'status' => 'approved', // ここが重要
                'applied_at' => now(), // 申請日時を現在時刻として記録
            ]);

            $isApprovedAction = false; // 修正アクションなのでfalseのまま
        }
    ...
    public function showAdminAttendanceList(Request $request){
        ...
        // その日の勤怠データを取得
        // with('user', 'breaks') で関連するユーザーと休憩データをEager Load
