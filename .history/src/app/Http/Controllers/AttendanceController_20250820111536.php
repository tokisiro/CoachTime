...
//勤怠一覧画面関連
    ...
    public function showAttendanceList(Request $request){
        ...
        $attendances = Attendance::where('user_id', $userId)->whereYear('date', $selectedMonth->year)->whereMonth('date', $selectedMonth->month)->orderBy('date', 'asc')->with(['user', 'breaks']) // userとbreaksリレーションをロード
        ->get()
        ->map(function ($attendance) { // ★追加・修正★
                // total_break_minutes を HH:mm 形式に変換
                $attendance->formatted_total_break = $this->convertMinutesToHhMm($attendance->total_break_minutes);

                // working_minutes を HH:mm 形式に変換
                $attendance->formatted_working_time = $this->convertMinutesToHhMm($attendance->working_minutes);

                return $attendance;
            });

        return view('normal.attendance_list', compact('attendances'));
    }

    // ★追加: 分をHH:mm形式に変換するプライベートメソッド★
    private function convertMinutesToHhMm(?int $minutes): string
    {
        if (is_null($minutes)) {
            return '-'; // null の場合はハイフンなどを返す
        }
    ...
    public function showByDate(Request $request)
    {
        ...
        $attendances = Attendance::where('user_id', $userId)->where('id',$attendanceId)->with(['user', 'breaks'])
        ->first(); // 特定の日付なのでfirst()
        ...
        return view('normal.attendance_detail', compact('attendances')); // 詳細ビューを用意する想定
    }
}
