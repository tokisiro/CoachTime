...
//勤怠一覧画面関連
    ...
    public function showAttendanceList(Request $request){
        ...
        $attendances = Attendance::where('user_id', $userId)->whereYear('date', $selectedMonth->year)->whereMonth('date', $selectedMonth->month)->orderBy('date', 'asc')->with(['user', 'breaks']) // userとbreaksリレーションをロード->get();
    ...
    {
        $userId = Auth::id();
        $date = $request->input('date') ?? now()->toDateString(); // 日付が指定されなければ今日の日付

        $attendances = Attendance::where('user_id', $userId)->where('date', $date)->with(['user', 'breaks'])
        ->first(); // 特定の日付なのでfirst()
