...
class AttendanceController extends Controller
{
    ...
    public function recordAttendance(Request $request)
    {
        ...
        // 今日の出勤記録を検索
        $attendance = Attendance::where('users_id', $user_id)
                                ->where('date', $today)
                                ->first();
    ...
    public function recordClosingTime(Request $request)
    {
        ...
        if ($userId) {
            ...
            $attendanceRecord = Attendance::where('users_id', $user_id)
    ...
    public function recordBreakIn(Request $request)
    {
        ...
        // 今日の出勤レコードを取得または作成
        $attendance = Attendance::where('usersID' , $userId) ->whereDate('date', Carbon::today())->first();
        ...
        // 新しい休憩レコードを作成
        $break = new BreakModel(); // BreakModelを使用
    ...
    public function recordBreakBack(Request $request)
    {
        ...
        return response()->json(['status' => 'error', 'message' => '開始中の休憩が見つかりません。'], 400);
    }
}
