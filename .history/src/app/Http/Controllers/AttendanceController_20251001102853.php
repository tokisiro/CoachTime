...
//出勤登録画面関連
    ...
    public function showAttendanceRegister()
    {
        ...
        // 現在の日時情報を取得し、ビューに渡す
        $now = Carbon::now();
        $dateStr = $now->isoFormat('YYYY年MM月DD日(ddd)');
        $timeStr = $now->format('H:i');

        return view('normal.attendance_register', compact('currentStatus', 'hasActiveBreak', 'dateStr', 'timeStr'));
    }
    ...
    public function recordAttendance(Request $request)
    {
        ...
        if (!$attendance) {
            ...
            session()->flash('error', 'すでに出勤記録があります。');
        }
        return redirect()->route('attendance.register'); // ページをリダイレクトして再描画
    }
    ...
    public function recordClosingTime(Request $request)
    {
        ...
        $currentDate = Carbon::today();
            ...
            if ($activeBreak) {
                session()->flash('error', '休憩が終了していません。');
                return redirect()->route('attendance.register');
            }

            $attendance->update([
                'closing_time' => Carbon::now(), // カラム名を 'closing_time' に変更
            ]);
            }
            ...
            if($attendance->check_in_time && $attendance->closing_time) {
                ...
                $checkOut = Carbon::parse($attendance->closing_time);
            ...
            //休憩時間の合計を計算
                ...
                $attendance->working_minutes = $totalMinutes - $breakMinutes;
                $attendance->save(); // 再度保存
            }
    ...
    public function recordBreakBack(Request $request)
    {
        ...
        $attendance = Attendance::where('user_id', $userId)
            ->whereDate('date', $currentDate)
            ->whereNotNull('check_in_time') // 出勤済みであること
            ->whereNull('closing_time')
            ->first();
...
//勤怠一覧画面関連
    ...
    public function showAttendanceList(Request $request){
        ...
        $attendances->map(function ($attendance) {
            ...
            $attendance->formatted_total_break_time = $this->convertMinutesToHhMm($totalBreakMinutes, true);
            ...
            if ($checkIn && $closing) {
                ...
                $attendance->working_minutes = $this->convertMinutesToHhMm($workingMinutes, true);
            } else {
    ...
    private function convertMinutesToHhMm(?int $minutes, bool $returnEmptyIfZero = false): string
    {
        ...
        if ($returnEmptyIfZero && $minutes === 0) {
            return ''; // 0 分で、かつ空文字列を返す設定の場合
        }
...
//勤怠詳細画面関連
    ...
    public function showDetails(Request $request, $id)
    {
        ...
        // latestApprovedApplication が存在しない場合のダミーデータ
        if (!$latestApprovedApplication) {
            $latestApprovedApplication = (object) [
                'proposed_check_in_time' => null,
                'proposed_closing_time' => null,
                'proposed_remarks' => null,
                'proposed_breaks' => '[]', // JSON文字列として初期化
        ...
        // 各フィールドの value を決定
        // check_in_time
        $checkInTimeValue = old('check_in_time',
            $isPendingApproval
                ? $formatTime($pendingApplication->proposed_check_in_time)
                : (
                    $formatTime($latestApprovedApplication->proposed_check_in_time) ?: $formatTime($attendance->check_in_time)
                )
        );

        // closing_time
        $closingTimeValue = old('closing_time',
            $isPendingApproval
                ? $formatTime($pendingApplication->proposed_closing_time)
                : (
                    $formatTime($latestApprovedApplication->proposed_closing_time) ?: $formatTime($attendance->closing_time)
                )
        );

        // remarks
        $remarksValue = old('proposed_remarks',
            $isPendingApproval
                ? ($pendingApplication->proposed_remarks ?? '')
                : ($latestApprovedApplication->proposed_remarks ?? $attendance->remarks ?? '')
        );

        // 既存休憩時間の value を各 break オブジェクトに追加
        $attendance->breaks->each(function ($break) use ($isPendingApproval, $pendingApplication, $formatTime) {
            // 申請中の休憩データを取得（もしあれば）
            $proposedBreak = collect($pendingApplication->decoded_proposed_breaks)->firstWhere('id', $break->id);

            $break->start_time_value = old("breaks.{$break->id}.start_time",
                $isPendingApproval && $proposedBreak && isset($proposedBreak['start_time'])
                    ? $formatTime($proposedBreak['start_time'])
                    : $formatTime($break->start_time) // もともとの休憩時間
            );

            $break->end_time_value = old("breaks.{$break->id}.end_time",
                $isPendingApproval && $proposedBreak && isset($proposedBreak['end_time'])
                    ? $formatTime($proposedBreak['end_time'])
                    : $formatTime($break->end_time) // もともとの休憩時間
            );
        });

        // 新規休憩時間の value
        $newBreakStartTimeValue = old('new_breaks.0.start_time',
            $isPendingApproval && !empty($pendingApplication->new_proposed_breaks[0])
                ? $formatTime($pendingApplication->new_proposed_breaks[0]['start_time'])
                : (
                    (!empty($latestApprovedApplication->new_proposed_breaks[0])
                        ? $formatTime($latestApprovedApplication->new_proposed_breaks[0]['start_time'])
                        : ''
                    )
                )
        );
        $newBreakEndTimeValue = old('new_breaks.0.end_time',
            $isPendingApproval && !empty($pendingApplication->new_proposed_breaks[0])
                ? $formatTime($pendingApplication->new_proposed_breaks[0]['end_time'])
                : (
                    (!empty($latestApprovedApplication->new_proposed_breaks[0])
                        ? $formatTime($latestApprovedApplication->new_proposed_breaks[0]['end_time'])
                        : ''
                    )
                )
        );
        ...
        $data = compact(
            ...
            'isRedirectedApproved',
            'checkInTimeValue',
            'closingTimeValue',
            'remarksValue',
            'newBreakStartTimeValue',
            'newBreakEndTimeValue'
        );

        // normal.attendance_details 専用のデータ準備
        if (!$isAdminLoggedIn) {
            // 時間フォーマット用のヘルパー関数
            $formatTime = function($time) {
                return $time ? Carbon::parse($time)->format('H:i') : '';
            ...
            // check_in_time
            $data['defaultCheckInTime'] = old('check_in_time');
            if (empty($data['defaultCheckInTime'])) {
                if ($isPendingApproval && $pendingApplication->proposed_check_in_time) {
                    $data['defaultCheckInTime'] = $formatTime($pendingApplication->proposed_check_in_time);
                } elseif ($latestApprovedApplication && $latestApprovedApplication->proposed_check_in_time) {
                    $data['defaultCheckInTime'] = $formatTime($latestApprovedApplication->proposed_check_in_time);
                } else {
                    $data['defaultCheckInTime'] = $formatTime($attendance->check_in_time);
                }
            }
            // closing_time
            $data['defaultClosingTime'] = old('closing_time');
            if (empty($data['defaultClosingTime'])) {
                if ($isPendingApproval && $pendingApplication->proposed_closing_time) {
                    $data['defaultClosingTime'] = $formatTime($pendingApplication->proposed_closing_time);
                } elseif ($latestApprovedApplication && $latestApprovedApplication->proposed_closing_time) {
                    $data['defaultClosingTime'] = $formatTime($latestApprovedApplication->proposed_closing_time);
                } else {
                    $data['defaultClosingTime'] = $formatTime($attendance->closing_time);
                }
            }

            // new_breaks[0][start_time]
            $data['defaultNewBreakStartTime0'] = old('new_breaks.0.start_time');
            if (empty($data['defaultNewBreakStartTime0'])) {
                // $pendingApplication と $latestApprovedApplication の new_proposed_breaks は既に準備済み
                $pendingNewBreak0 = $pendingApplication->new_proposed_breaks[0] ?? null;
                $approvedNewBreak0 = $latestApprovedApplication->new_proposed_breaks[0] ?? null;

                if ($isPendingApproval && $pendingNewBreak0 && ($pendingNewBreak0['start_time'] ?? null)) {
                    $data['defaultNewBreakStartTime0'] = $formatTime($pendingNewBreak0['start_time']);
                } elseif ($approvedNewBreak0 && ($approvedNewBreak0['start_time'] ?? null)) {
                    $data['defaultNewBreakStartTime0'] = $formatTime($approvedNewBreak0['start_time']);
                }
            }

            // new_breaks[0][end_time]
            $data['defaultNewBreakEndTime0'] = old('new_breaks.0.end_time');
            if (empty($data['defaultNewBreakEndTime0'])) {
                $pendingNewBreak0 = $pendingApplication->new_proposed_breaks[0] ?? null;
                $approvedNewBreak0 = $latestApprovedApplication->new_proposed_breaks[0] ?? null;

                if ($isPendingApproval && $pendingNewBreak0 && ($pendingNewBreak0['end_time'] ?? null)) {
                    $data['defaultNewBreakEndTime0'] = $formatTime($pendingNewBreak0['end_time']);
                } elseif ($approvedNewBreak0 && ($approvedNewBreak0['end_time'] ?? null)) {
                    $data['defaultNewBreakEndTime0'] = $formatTime($approvedNewBreak0['end_time']);
                }
            }

            // remarks
            $data['defaultRemarks'] = old('remarks');
            if (empty($data['defaultRemarks'])) {
                if ($isPendingApproval) {
                    $data['defaultRemarks'] = $pendingApplication->proposed_remarks ?? '';
                } elseif ($latestApprovedApplication) {
                    $data['defaultRemarks'] = $latestApprovedApplication->proposed_remarks ?? '';
                } else {
                    $data['defaultRemarks'] = $attendance->remarks ?? '';
                }
            }
        }
    ...
    public function applications(DetailsRequest $request, $id)
    {
        ...
        try {
            ...
            $application = Application::create([
                ...
                'proposed_remarks' => $request->input('proposed_remarks'),
            ]);
    ...
    //修正申請承認機能(管理)
    public function approve(AdminDetailsRequest $request, $id)
    {
        ...
        if ($application) {
            ...
            foreach ($application->breaks as $proposedBreak) {
                ...
                if ($proposedBreak->proposed_start_time !== null || $proposedBreak->proposed_end_time !== null) {
                    ...
                    $proposedBreak->proposed_start_time = null;
                    $proposedBreak->proposed_end_time = null;
                    $proposedBreak->application_id = null;
                    $proposedBreak->save();
                }
            }
            // 申請のステータスを承認済みに変更
            ...
            $attendance->remarks = $request->input('proposed_remarks');
            $attendance->save();
    ...
    public function showStaffAttendance(Request $request, $user)
    {
        ...
        dd($attendances->toArray()); // ユーザーの全勤怠データとその breaks を確認
    dd($attendances->first()->breaks->toArray());
